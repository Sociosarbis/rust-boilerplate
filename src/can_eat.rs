  ...
  fn test_can_eat() {
    let suites = vec![
      Suite {
        ...
        queries: t2_i![[0,2,2],[4,2,4],[2,13,1000000000]],
        ret: vec![true,false,true]
      },
      Suite {
        candies_count: vec![5,2,6,4,1], 
        queries: t2_i![[3,1,2],[4,10,3],[3,10,100],[4,100,30],[1,3,1]],
      ...
      Suite {
        ...
        queries: t2_i![[85, 54, 42]],
        ret: vec![true]
      }
    ];

    for s in suites {
      assert_eq!(s.ret, Solution::can_eat(s.candies_count, s.queries));
    }
  }
}